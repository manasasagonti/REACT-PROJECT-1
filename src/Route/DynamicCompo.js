import { useParams , Link , useNavigate } from "react-router-dom";
import { bucket } from "../Store/CreateStore";
import { useContext  } from "react";


function DynamicCompo (){
    const data = useParams()
    const [arr] = useContext(bucket)
    const Navi = useNavigate()
    
    
    const filterBlog = arr.filter((item) => item.id === Number(data.id))
    let data2 = arr.filter((item) => item.cat === filterBlog[0].cat).filter((item) => item.id !== filterBlog[0].id)
    console.log(data2);
   
    return(
        <div className="parent_blog">
            <div className="rightchild_blog">
            <h1 className="blogtitle">{filterBlog[0].title}</h1>
             <div className="user_info">
                <div className="user_profile">

                <img className="user_image" src="data:image/png;base64,"/>
                 <p>Manasa Sagonti</p>
              </div>  
              <i className="fa-brands fa-instagram"></i>
              <i className="fa-brands fa-facebook"></i>
              <i className="fa-brands fa-twitter"></i>
                
                </div>
                
                <img className="rightchild_image" src={filterBlog[0].img} alt="not found"></img>
                <p className="rightchild_info">{filterBlog[0].content}</p>
                <div className="user_info">
                <div className="user_profile">

                <img className="user_image" src="data:image/png;base64,"/>
                 <p>Manasa Sagonti</p>
              </div>  
                </div>   
            </div>
            <button className="back_button" onClick={() => Navi(-1)}>Go Back</button>
            <h1 className="morefromsiren_heading">More from Siren</h1>
            <div className="morefrom_siren">
            {data2.map((item,index) => {
                if(index <= 3){
                    return <div className="latest_section1">
                <img src={item.img} className="image_home" alt="image not found"></img><hr></hr>
                <div className="home_content">
                    <h3 className="title_link" ><Link to={`/details/${item.id}`}>{item.title.slice(0,60)}</Link></h3>
                    <p className="date">Bollywood : 14-10-23</p>
                </div>
                </div>
            }
                
})}
            </div>
        </div>
    )
}
export default DynamicCompo;